...
export const generateFile_node = ({
  language,
  code,
}: {
  language: number;
  ...
  if (
    ...
    language_id[language as keyof typeof language_id] != "js"
  ) {
    return {
      file_path: "",
      jobID: null,
...
export const generate_input_file_node = (fsname: string, info: string) => {
...
export const execute_code_node = ({
  filepath,
  jobid,
  input,
}: {
  ...
  try {
    const { input_file_path } = generate_input_file_node(`${jobid}.txt`, input);
    return new Promise((reject, resolve) => {
      exec(`node ${filepath} < ${input_file_path}`, (error, stderr, stdout) => {
           if (error) {
            reject({
              type: "error",
